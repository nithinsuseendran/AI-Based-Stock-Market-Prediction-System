...
export default function RegisterPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to StockAI Pro')
      navigate('/dashboard')
    } catch (err) {
      toast.error(err?.response?.data?.detail || 'Registration failed. Running in demo mode.')
      navigate('/dashboard')
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg-primary)] px-4">
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[400px] rounded-full bg-violet-600/10 blur-[120px] pointer-events-none" />
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Join thousands of smarter investors</p>
        </div>
